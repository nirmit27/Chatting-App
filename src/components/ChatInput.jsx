...
export default function ChatInput({ onNewMessage }){
    ...
    useEffect(() => {
        ...
        return () => {
            form.removeEventListener('submit', handleSubmit)    // to prevent MEMORY LEAKS if the component is unmounted/removed from DOM
        }
    }, [handleSubmit])

    return (
        <form id="form" action="" method="post" class="fixed bottom-0 flex max-h-24 w-screen justify-center bg-slate-300 p-6">
        <input type="text" name="chat-input" id="chat-input" placeholder="Enter your prompt" value={inputValue} onChange={handleChange}
